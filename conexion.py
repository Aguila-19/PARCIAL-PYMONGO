# GUI CRUD + LOGIN (Tkinter + PyMongo + bcrypt)
# BD: Tienda | Cols: Usuarios, Productos
# Login previo -> si ok, abre el CRUD (Código de producto / En existencia)
...
def conectar():
    ...
    col_prod.create_index([("codigo", ASCENDING)], unique=True)
...
def open_login_window(on_success):
    ...
    def do_login():
        ...
        if not check_password(password, user["password"]):
            ...
            return
        lw.destroy()
        on_success(user)  # pasa el dict del usuario logueado
...
# ------------- CRUD (mismo GUI, con “Código de producto” / “En existencia”) -------------
...
def listar_todos():
    cursor = col_prod.find({}, {"_id": 0}).sort("nombre", ASCENDING)
...
def buscar():
    ...
    if pmin is not None or pmax is not None:
        ...
        filtro["precio"] = rango
    cursor = col_prod.find(filtro, {"_id": 0}).sort("nombre", ASCENDING)
    refresh_table(cursor)
